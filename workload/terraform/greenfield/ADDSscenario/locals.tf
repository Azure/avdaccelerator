locals {
  ...
  # setSessionHost_filePath = "../../../scripts/${local.setSessionHost_fileName}"

  #powershell_script_rendered                = filebase64(local.setSessionHost_filePath)
  # setSessionHost_commandToExecute_LocalFile = "powershell.exe -command \"[System.Text.Encoding]::UTF8.GetString([System.Convert]::FromBase64String('${local.powershell_script_rendered}')) | Out-File -filepath ${local.setSessionHost_fileName}\" && powershell -ExecutionPolicy Unrestricted -File ${local.setSessionHost_fileName} ${local.parameters} -verbose"
