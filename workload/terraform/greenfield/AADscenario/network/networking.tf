...
resource "azurerm_virtual_network_peering" "peer1" {
  ...
  resource_group_name          = azurerm_resource_group.net.name
  virtual_network_name         = azurerm_virtual_network.vnet.name
  remote_virtual_network_id    = data.azurerm_virtual_network.remote.id //data.azurerm_virtual_network.ad_vnet_data.id
...
resource "azurerm_virtual_network_peering" "peer2" {
  ...
  remote_virtual_network_id    = data.azurerm_virtual_network.vnet.id //azurerm_virtual_network.vnet.id
  allow_virtual_network_access = true
  allow_forwarded_traffic      = true
  provider                     = azurerm.hub
}
