...
- Private DNS zones for Azure files (privatelink.file.core.windows.net) and keyvault (privatelink.vaultcore.azure.net) private endpoints name resolution. The private DNS zones will need to be linked to the AVD subnet when not using custom DNS servers, or to the vNet where the custom DNS servers are connected where they are configured on the AVD vNet.
