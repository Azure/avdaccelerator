            ...
            Script AAD-NTFS-Permissions {
                ...
                SetScript  = {
                    ...
                    try {
                        ...
                        $ErrMsg = $PSItem | Format-List -Force | Out-String
                        Write-Log -Err $ErrMsg
                        throw [System.Exception]::new("Some error occurred in DSC AAD NTFS SetScript: $ErrMsg", $PSItem.Exception)
                    }
                }
                TestScript = {
                    . (Join-Path $using:ScriptPath "Logger.ps1")
                    try {
                        Write-Log "AAD NTFS permissions on $Using:StorageAccountName configured."
                    }
                    catch {
                        $ErrMsg = $PSItem | Format-List -Force | Out-String
                        Write-Log -Err $ErrMsg
