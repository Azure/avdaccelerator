...
param avdSessionHostBatchCount int

@description('Max number of resources per template dpeloyment.')
param avdMaxResourcesPerTemplateDeployment int
...
// Session hosts.
@batchSize(1)
module avdSessionHosts '../../../carml/1.2.0/Microsoft.Compute/virtualMachines/deploy.bicep' = [for i in range(1, avdSessionHostBatchCount):  {
  scope: resourceGroup('${avdWorkloadSubsId}', '${avdComputeObjectsRgName}')
  name: 'AVD-Session-Host-${i-1}-${time}'
  params: {
      ...
      //avdDeploySessionHostsCount: i == avdDeploySessionHostsCount && divisionRemainderValue > 0 ? divisionRemainderValue : MaxResourcesPerTemplateDeployment
      //avdSessionHostCountIndex: i == 1 ? avdSessionHostCountIndex : ((i - 1) * MaxResourcesPerTemplateDeployment) + avdSessionHostCountIndex
